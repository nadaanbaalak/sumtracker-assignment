...
export const createPaginationString = (
  ...
  const nonPaginatedCount =
    pagination.resultsCount ?? pagination.count ?? limit;
  ...
  const limitString = pagination.hasOffset
...
const ResultString: FC<ResultStringInterface> = ({
  pagination,
  loading,
  pageString,
}) => {
  return (
    <div>
      <Typography.Text>
        {createPaginationString(pagination, loading, pageString)}
      </Typography.Text>
    </div>
  );
};
